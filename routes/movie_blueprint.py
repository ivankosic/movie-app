...
def search():
    ...
    query = """
        FOR m IN movie
    FILTER
        CONTAINS(LOWER(m.title), LOWER(@term)) OR
        CONTAINS(LOWER(m.description), LOWER(@term)) OR
        CONTAINS(LOWER(TO_STRING(m.releaseDate)), LOWER(@term)) OR
        CONTAINS(TO_STRING(m.runningTime), @term) OR
        CONTAINS(LOWER(m.imageUrl), LOWER(@term)) OR
        CONTAINS(TO_STRING(m.rating), @term)
    RETURN {
        _key: m._key,
        title: m.title,
